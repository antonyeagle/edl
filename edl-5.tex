...
\begin{definition}[Disjunctive Normal Form] A sentence is in \emph{Disjunctive Normal Form} (\textsc{\lowercase{DNF}}) if it contains only connectives drawn from $\{\neg,\wedge,\vee\}$, and such that $\vee$ never occurs in the scope of $\wedge$ or $\neg$, and $\wedge$ never occurs in the scope of $\neg$. \end{definition}
...
\begin{theorem}[\textsc{\lowercase{DNF}}]\label{\textsc{\lowercase{dnf}}}
...
~\boxer{Duality}{The \emph{dual} of a truth-functional connective is that connective whose truth table
...
\end{enumerate}
\begin{lemma}[Duality for $\wedge,\vee$]
...
\paragraph{Duality Theorem}
 ...
 \begin{theorem}[Duality for $\wedge,\vee$]
...
\begin{theorem}[Decidability of Finite Validity in \lone]
...
\begin{theorem}[Positive Decidability of Inconsistency in \lone]
